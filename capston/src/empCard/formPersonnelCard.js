...
const App = ({ componentRef, sabun }) => {
  ...
  useEffect(() => {
    ...
    axios
      .post("http://43.200.115.198:8080/getLicense.jsp", postParam)
      .then((res) => {
    ...
    axios
      .post("http://43.200.115.198:8080/getBank.jsp", postParam)
      .then((res) => {
        let data = res.data.ITEMS;
        setGetBank(data);
      });
    ...
    axios
      .post("http://43.200.115.198:8080/getFamily.jsp", postParam)
      .then((res) => {
        let data = res.data.ITEMS;
        setFamily(data);
      })
    ...
    postParam = qs.stringify(postParam);
    axios
      .post("http://43.200.115.198:8080/getAppointment.jsp", postParam)
      .then((res) => {
        let data = res.data.ITEMS;
        setAppointment(data);
      })
        ...
        <table>
          ...
          <tbody>
            <tr className="firsttable">
              <td
                rowSpan={4}
                style={{ padding: "0px", minWidth: "125px", height: "180px" }}
              >
                ...
                {appointment.map((item, index) => {
                  return (
                    <tr>
                      ...
                      <td>
                        {item.app_date.slice(0, 4) +
                          "년 " +
                          item.app_date.slice(4, 6) +
                          "월 " +
                          item.app_date.slice(6, 8) +
                          "일"}
                      </td>
                      <td>{item.deptKR}</td>
                      <td>{item.rankKR}</td>
