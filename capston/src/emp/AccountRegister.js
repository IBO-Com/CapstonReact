...
const Account = () => {
  ...
  const saveBtnClick = () => { //저장 버튼
    let familyTable = document.querySelectorAll(".AccountRegister_table tbody tr");
    let isReady = true;
    let data = [];
    for(let i = 0; i < familyTable.length; i ++) {
      console.log("f" , familyTable[i]);
      let tempArr = [];
      for(let j = 2; j <= 4; j++) {
        let element = null; 
        if(j == 2) {
    ...
    <div className="accountRegister_container">
      <div className="accountRegister_title">
        ...
        <div className="accountRegister_Btn">
          ...
          <button className="accountRegister_saveBtn" onClick={() => {saveBtnClick()}}>저장</button>
        </div>
      </div>
      <div className="AccountRegister_table">
        <table>
          <thead>
            <tr>
              <td>번호</td>
